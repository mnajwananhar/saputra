...
const Dashboard: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const lowStockProducts = products.filter(
      (p) => p.stock.currentStock <= p.stock.minStock,
    );
    ...
    const totalSold = thisMonthTransactions.reduce(
      (sum, t) =>
    ...
    const topProducts = products
      .map((p) => {
        const sold = thisMonthTransactions.reduce(
          (sum, t) =>
            sum + (t.items.find((i) => i.productId === p.id)?.quantity || 0),
          0,
        );
        return { ...p, sold };
      })
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-700">
      <div>
        <h1 className="text-5xl font-heading font-extrabold text-slate-900 tracking-tighter leading-none uppercase">
          Dashboard
        </h1>
        <p className="text-slate-400 mt-3 text-[10px] font-bold uppercase tracking-[0.3em]">
          Overview & Quick Stats - {currentMonth}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="bg-white p-7 rounded-[1.75rem] border border-slate-200 shadow-sm">
          ...
          <div
            className={`text-4xl font-heading font-extrabold tracking-tighter ${stats.lowStockProducts.length > 0 ? "text-red-500" : "text-green-500"}`}
          >
      ...
      {stats.topProducts.length > 0 && (
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-100">
            <h3 className="font-heading font-extrabold text-slate-900 text-xl tracking-tighter uppercase">
            ...
            {stats.topProducts.map((product, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-heading font-extrabold text-slate-900 text-sm tracking-tight uppercase">
                      {product.name}
                    </div>
                ...
                <div className="text-right">
                  <div className="text-xl font-heading font-extrabold text-indigo-600 tracking-tighter">
                    {product.sold}
                  </div>
        ...
        <div className="bg-white rounded-[2rem] border border-red-200 shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-red-100 bg-red-50/30">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-heading font-extrabold text-red-900 text-xl tracking-tighter uppercase">
                Alert: Stok Rendah
              </h3>
            </div>
            <p className="text-[10px] text-red-400 font-bold uppercase tracking-widest mt-0.5">
              Produk yang perlu segera diorder
            </p>
          </div>
          <div className="p-6 space-y-3 max-h-80 overflow-y-auto">
            {stats.lowStockProducts.map((product) => (
              <div
                key={product.id}
                className="flex items-center justify-between p-4 bg-red-50/30 rounded-xl border border-red-100"
              >
                <div>
                  <div className="font-heading font-extrabold text-slate-900 text-sm tracking-tight uppercase">
                    {product.name}
                ...
                <div className="text-right">
                  <div className="text-xl font-heading font-extrabold text-red-500 tracking-tighter">
