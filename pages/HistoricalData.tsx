...
import {
  aggregateTransactionsToMonthlyWithDetails,
...
const HistoricalData: React.FC = () => {
  ...
  const aggregatedData = useMemo(() => {
    ...
    products.forEach((product) => {
      ...
      const productTransactions = transactions.filter((transaction) => {
        const hasProduct = transaction.items.some(
      ...
      result[product.id] =
        aggregateTransactionsToMonthlyWithDetails(productTransactions);
    });
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      "Periode",
      ...filteredProducts.map((p) => `${p.name} (${p.unit})`),
    ];
    ...
    const rows = sortedPeriods.map((period) => {
      const rowData = [
        period,
        ...products.map((p) => {
          const periodData = aggregatedData[p.id].find(
            (d) => d.periodLabel === period,
          );
    ...
    link.setAttribute(
      "download",
      `data_penjualan_saputra_jaya_${selectedYear}_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const toggleExpand = (productId: string, period: string) => {
    if (
      expandedCell?.productId === productId &&
      expandedCell?.period === period
    ) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8 bg-slate-50 rounded-[2.5rem] border border-slate-200 border-dashed">
        ...
        <p className="text-sm text-slate-500 mt-2 max-w-md">
          Belum ada data historis yang tersedia. Silakan tambahkan produk dan
          lakukan transaksi.
        </p>
      </div>
    );
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div>
            <h1 className="text-5xl font-heading font-extrabold text-slate-900 tracking-tighter leading-none">
              Data Historis
            </h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-2">
              Historical Sales Record • Klik sel untuk detail harian
            </p>
          </div>
      ...
      <div className="rounded-[2.5rem] border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-auto max-h-[600px]">
          <table className="w-full text-left border-separate border-spacing-0">
            <thead className="bg-slate-50/50 text-slate-400 sticky top-0 z-20">
              <tr>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-[0.2em] sticky left-0 bg-slate-100 z-30 min-w-[200px] border-b border-r border-slate-200">
                  Produk
                </th>
                {sortedPeriods.map((period, index) => (
                  <th
                    key={index}
                    className="px-4 py-4 text-center text-[10px] font-bold uppercase tracking-[0.2em] whitespace-nowrap bg-slate-50 border-b border-slate-200 min-w-[80px]"
                  >
                    {period}
                  </th>
                ))}
              ...
              {products.map((p, rowIndex) => (
                ...
                >
                  ...
                  {sortedPeriods.map((period, index) => {
                    ...
                    const isExpanded =
                      expandedCell?.productId === p.id &&
                      expandedCell?.period === period;
                    const hasDailyData =
                      periodData &&
                      periodData.dailyTransactions &&
                      periodData.dailyTransactions.length > 0;
                    ...
                    return (
                      <td
                        ...
                        onClick={() =>
                          hasDailyData && toggleExpand(p.id, period)
                        }
                      >
                        <div className="flex items-center justify-center gap-1">
                          <span
                            className={`tabular-nums font-heading font-extrabold text-lg tracking-tighter ${hasDailyData ? "text-indigo-600" : "text-slate-600"}`}
                          >
                        ...
                        {isExpanded && periodData && (
                          ...
                          >
                            ...
                            <div className="space-y-2 max-h-[200px] overflow-auto">
                              {periodData.dailyTransactions.map(
                                (daily, dIdx) => (
                                  <div
                                    key={dIdx}
                            ...
                            <div className="mt-3 pt-2 border-t border-slate-100 flex justify-between items-center">
                              ...
                              <span className="font-heading font-extrabold text-indigo-600 tabular-nums">
                                {periodData.demand} {p.unit}
                              </span>
                            </div>
                          </div>
                        )}
        ...
        <div className="bg-slate-50/30 p-6 border-t border-slate-100">
          <p className="text-[10px] text-center text-slate-400 uppercase tracking-[0.2em]">
            Data diambil dari transaksi penjualan harian • {products.length}{" "}
            produk • {sortedPeriods.length} periode • Klik angka untuk detail
          </p>
        </div>
      </div>
    </div>
